...
public class BltManager {
    ...
    public interface OnRegisterBltReceiver {
        ...
        void onBltMatchDevice(BluetoothDevice device);
     ...
     * 蓝牙完成状态接口
     */
    private OnRegisterBltFinishReceiver mOnRegisterBltFinishReceiver;

    public interface OnRegisterBltFinishReceiver {
        void onBlueFinish();
    ...
    public void registerBltReceiver(Context context, OnRegisterBltReceiver onRegisterBltReceiver) {
        ...
        //intent.addAction(BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE);//请求用户选择是否使本机蓝牙能被扫描
        //intent.addAction(BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED);//请求设备name发生改变
     ...
     * @param context
     */
    public void registerBltReceiver2(Context context, OnRegisterBltFinishReceiver onRegisterBltFinishReceiver) {
        this.mOnRegisterBltFinishReceiver = onRegisterBltFinishReceiver;
        // 用BroadcastReceiver来取得搜索结果
        IntentFilter intent = new IntentFilter();
        intent.addAction(BluetoothAdapter.ACTION_DISCOVERY_FINISHED);//扫描完成
        ...
        //intent.addAction(BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED);//请求设备name发生改变
        //intent.addAction(BluetoothAdapter.ACTION_CONNECTION_STATE_CHANGED);//连接状态改变
        context.registerReceiver(finishReceiver, intent);
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
                    ...
                    Log.i("blu", "设备已经配对过");
                    onRegisterBltReceiver.onBltMatchDevice(device);
                }
            } else if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
                //状态改变时
                device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                Log.i("blu", "状态改变时" + device.getName());
                ...
                Log.i("blu", "蓝牙设备name发生改变");
            } else if (BluetoothAdapter.ACTION_CONNECTION_STATE_CHANGED.equals(action)) {
                device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                Log.i("blu", "蓝牙连接状态改变" + device.getName());
            } else if (BluetoothAdapter.ACTION_REQUEST_DISCOVERABLE.equals(action)) {
                Log.i("blu", "请求本地蓝牙设备能否被搜索");
            }
        }
    };

    /**
     * 搜索蓝牙完成广播&手机本身蓝牙状态改变广播
     */
    private BroadcastReceiver finishReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action)) {
                Log.i("blu", "搜索完成");
                mOnRegisterBltFinishReceiver.onBlueFinish();
            }
            ...
            if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
                ...
                switch (state) {
                    case BluetoothAdapter.STATE_OFF:
                        Log.i("blu", "STATE_OFF 手机蓝牙关闭");
                        mOnRegisterBltFinishReceiver.onCloseBlu();
                        break;
                    case BluetoothAdapter.STATE_ON:
                        Log.i("blu", "STATE_ON 手机蓝牙开启");
                        mOnRegisterBltFinishReceiver.onOpenBlu();
                        break;
                    case BluetoothAdapter.STATE_TURNING_ON:
                        Log.i("blu", "STATE_TURNING_ON 手机蓝牙正在开启");
                        mOnRegisterBltFinishReceiver.onOpening();
                        break;
                    case BluetoothAdapter.STATE_TURNING_OFF:
                        Log.i("blu", "STATE_TURNING_OFF 手机蓝牙正在关闭");
                        mOnRegisterBltFinishReceiver.onClosing();
                        break;
                }
            }
    ...
    public int bltStatus(int status) {
        //String a = "未知状态";
        int a = 0;
        switch (status) {
    ...
    private void connect(BluetoothDevice btDev, Handler handler) {
        try {
            ...
            if (!getmBluetoothSocket().isConnected()) {
                ...
                int bondState = btDev.getBondState();
                Log.i("blu", "-----  " + bondState);
                try {
    ...
    public void clickBlt(Context context, int status) {
        switch (status) {
            ...
            case BltConstant.BLUE_TOOTH_CLOSE://本机蓝牙禁用
                ...
                break;
            case BltConstant.BLUE_TOOTH_MY_SEARCH://本机蓝牙可以在300s内被搜索到
    ...
    public void checkBleDevice(Context context) {
        BluetoothAdapter bluetoothAdapter = getmBluetoothAdapter();
